...
async def async_setup_entry(hass: HomeAssistant, entry: ConfigEntry) -> bool:
    ...
    coordinator = RainVisionCdcCoordinator(hass, client, entry.entry_id)
    await coordinator.async_setup()
    coordinator.set_poll_window(
        entry.options.get(CONF_POLL_WINDOW_START, ""),
        entry.options.get(CONF_POLL_WINDOW_END, ""),
    )
