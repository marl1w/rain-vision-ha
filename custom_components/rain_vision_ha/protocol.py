...
def set_icycle_active(raw: bytes, active: bool) -> bytes:
    """Return a copy of a raw iCycle record with the active bit set or cleared.

    Bit 0 of byte 1 is the active flag (info_bits[14] in decode_icycle).
    """
    data = bytearray(raw)
    if active:
        data[1] |= 0x01
...
def next_run(schedule: RainVisionSchedule, now: datetime) -> datetime | None:
    ...
    # The controller uses 00:00 for disabled/non-scheduled cycles.
...
class RainVisionCdcClient:
    ...
    async def read_icycles(self) -> list[RainVisionSchedule]:
        """Read only the iCycle records from the device."""
        ...
        try:
            ...
            schedules: list[RainVisionSchedule] = []
            if status.number_of_icycles > 0:
                await self._write(client, COMMAND_UUID, "command reset icycle pointer", bytes([COMMAND_RESET_ICYCLE_POINTER]))
                await asyncio.sleep(0.2)
                for index in range(status.number_of_icycles):
                    with suppress(Exception):
                        payload = await self._read(client, ICYCLE_UUID, f"icycle {index + 1}")
                        schedules.append(decode_icycle(payload))
                    await asyncio.sleep(0.01)
                await asyncio.sleep(0.2)
                with suppress(Exception):
                    await self._write(client, COMMAND_UUID, "command reset icycle pointer", bytes([COMMAND_RESET_ICYCLE_POINTER]))
    ...
    async def write_icycles(self, raw_records: list[bytes]) -> None:
        """Write iCycle records to the device, replacing all stored schedules."""

        _LOGGER.debug("Writing %s iCycles to %s", len(raw_records), self.address)
        client = await self._connect()
        try:
            await self._authenticate(client)
            await self._write(client, COMMAND_UUID, "command stop", bytes([COMMAND_START_READY_MODE]))
            await asyncio.sleep(0.1)
            await self._write(client, COMMAND_UUID, "command clear icycles", bytes([COMMAND_CLEAR_ICYCLES]))
            await asyncio.sleep(0.1)
            for index, raw in enumerate(raw_records):
                await self._write(client, ICYCLE_UUID, f"icycle {index + 1}/{len(raw_records)}", raw)
